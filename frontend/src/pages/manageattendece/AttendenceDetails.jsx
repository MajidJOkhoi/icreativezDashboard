...
const AttendanceDetails = () => {
  ...
  const fetchAttendanceData = async (date) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/attendance/getMyMonthAttendanceById?userid=${id}&month=${month}&year=${year}`);
      ...
      <Card className="mt-2 w-full rounded-3xl shadow-sm shadow-green-50 max-w-sm sm:max-w-full">
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <div className="text-red-500">No Record Available for the selected month ... </div>
          ) : (
              ...
              <TableBody>
                {attendanceData.map((attendance) => (
                  <TableRow key={attendance._id}>
                    ...
                    <TableCell>
                      {attendance.duration?.hours} hrs {attendance.duration?.minutes} mins {attendance.duration?.seconds} secs
                    </TableCell>
                    <TableCell>
                      <span
